...
(function () {
  ...
  function loadFS(name, cb) {
    ...
    ajax.onreadystatechange = function () {
      if (ajax.readyState == 4 && ajax.status == 200) cb(ajax.responseText);
    };
    ajax.open("GET", name);
    ajax.send();
  }
  ...
  var Terminal = {
    ...
    tabComplete: function (text) {
      ...
      if (parts.length == 1) {
        ...
        last = pathParts.pop();
        dir =
          pathParts.length > 0 ? this.getEntry(pathParts.join("/")) : this.cwd;
    ...
    writeLink: function (e, str) {
      this.write(
        '<span class="' + e.type + '">' + this._createLink(e, str) + "</span>"
      );
    },
    ...
    _createLink: function (entry, str) {
      function typeLink(text, link) {
        return (
          '<a href="javascript:void(0)" onclick="typeCommand(\'' +
          text +
          "')\">" +
          link +
          "</a>"
    ...
    _prompt: function () {
      ...
      prompt.innerHTML = this.config.prompt(
        this.getCWD(),
    ...
    _typeKey: function (key) {
      ...
      if (!stdout || key < 0x20 || key > 0x7e || key == 13 || key == 9) return;
    ...
    _handleSpecialKey: function (key, e) {
      ...
      // Enter.
      else if (key == 13) this.returnHandler(stdout.innerHTML);
      // Up arrow.
      else if (key == 38) {
  ...
  term.init(CONFIG, "/data/json/sample0.json", COMMANDS, function () {
    term.enqueue("taogpt").enqueue("hola").enqueue("raiz").begin();
  });
